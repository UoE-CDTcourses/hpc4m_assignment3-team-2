...
double** UFinal = new double* [M+1]; // this creates a 1D array with a 1D array in each entry.
for (int i = 0; i < M+1; ++i){
...
for (int i = 0; i < 3; ++i) {   // this loop puts an array of size J on each of the three entries.
...
if (rank!=root){
MPI_Send(&U[0][0][0],J*(M+1),MPI_DOUBLE,root,rank,MPI_COMM_WORLD);
}
...
MPI_Recv(&UFinal[(J-2)*i][0],J*(M+1),MPI_DOUBLE,i,i,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}}
...
//Halo-swapping
  if (rank < size-1){
	MPI_Send(&U[2][J-2][1],M-1,MPI_DOUBLE,rank+1,2,comm);
}
	if (rank > 0){
	MPI_Send(&U[2][1][1],M-1,MPI_DOUBLE,rank-1,2,comm);
	MPI_Recv(&U[2][0][1],M-1,MPI_DOUBLE,rank-1,2,comm,MPI_STATUS_IGNORE);
}
	if (rank < size-1){
	MPI_Recv(&U[2][J-1][1],M-1,MPI_DOUBLE,rank+1,2,comm,MPI_STATUS_IGNORE);
}
...
for (int i=0; i<J; ++i){
  for (int j=1; j<M; ++j){
   ...
   // print out files for fixed times
    ...
    if (rank!=root){
    MPI_Send(&U[2][0][0],J*(M+1),MPI_DOUBLE,root,rank,MPI_COMM_WORLD);
    }
